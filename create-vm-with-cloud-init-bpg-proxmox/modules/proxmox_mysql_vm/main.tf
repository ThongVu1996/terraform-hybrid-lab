...
resource "proxmox_virtual_environment_file" "mysql_config" {
  ...
  source_raw {
    file_name = "mysql-init-${var.db_name}.yml"
    data      = <<-EOF
      #cloud-config
      ...
      runcmd:
        ...
        - mysql -e "CREATE USER IF NOT EXISTS '${var.db_user}'@'100.%' IDENTIFIED BY '${var.db_password}';"
        - mysql -e "GRANT ALL PRIVILEGES ON ${var.db_name}.* TO '${var.db_user}'@'100.%';"
        # Cho Localhost
        - mysql -e "CREATE USER IF NOT EXISTS '${var.db_user}'@'localhost' IDENTIFIED BY '${var.db_password}';"
